...
- **P9 (#64) human-legible run reports — end-to-end thin vertical (branch feat/p9-human-legible-reports; detail in `.otto/memory/*-64-*.json`).** (1) `templates/quality-report.md` REORDERED layperson-first: Verdict → What Changed/Why/How To Verify/What To Watch/What I Was Unsure About (plain prose, no SHAs) → `_Engineer detail below_` divider → Task Source/Evidence/Checklist/Gaps. H1 `# Otto quality report` is STABLE — it's the persistence marker. (2) `run-report.ts` `writeRunReport`/`readRunReport` (best-effort, never throw) + `hasRunReport(result)` (keys on the H1, not stage name — the implementer emits the report; afk/plan-mode emits none) + `report` artifact kind. `loop.ts` tracks the latest report-bearing `sr.result`, `finalizeManifest` writes `.otto/runs/<id>/report.md` + links it via `collectArtifacts` (existsSync, like review-followups). (3) NEW `report-explain.ts` pure `formatPlainReport(manifest, reportText|null)` (report prose first, then a run-facts footer; null → absent-note + facts) + `runExplain` mirroring `runInspect`; NEW `otto-explain` bin. **GOTCHA: `scripts/quality-report-samples.test.mjs` parses the contract's `## ` headings AND asserts every sample in `docs/quality-report-samples.md` carries all of them — changing contract sections REQUIRES updating both the test's expected list and all 4 samples.** Deferred: report-legibility rubric+eval signal, afk report coverage (`prompt.md` doesn't `@include` the contract), embedded before/after evidence (persist diff/screenshots), `otto-inspect --plain` alias.
