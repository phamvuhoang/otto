...
- **P7 (#62) context & token efficiency — slices 1–8 (detail in `.otto/memory/*-62-*.json`).** Telemetry-first; all pure + INERT-on-the-loop (loop wiring deferred). `context-report.ts` `analyzeContext(prompt)` segments a rendered prompt into commits/learnings/inputs/playbook (unrecognized text = playbook) + `estimateTokens`=ceil(chars/4); captured onto `StageRecord.contextBreakdown?`, surfaced by `otto-afk --context-report` (+token slope). `tokens.ts` `summarizeCacheEfficiency` — prefix caching is REPORT-ONLY (Otto spawns the `claude` CLI, can't inject `cache_control`). `memory.ts` `selectRelevantMemory`/`boundLearnings` (task-scope-ranked, char-budget cap, reports dropped). `iteration-compaction.ts` `parseCommitLog`/`compactCommits` (keep newest commits full, degrade older to subject-only past the budget). `read-dedup.ts` `recordRead` + FNV-1a `fingerprintContent` (length-prefixed → a collision only ever causes a SAFE re-spill; pure, returns a fresh ledger). `context-budget.ts` `assessContextBudget` (soft model-aware ceiling; recommends compacting the largest REDUCIBLE filler — commits/learnings, not inputs/playbook). All exported from index.ts; pinned by per-module `*.test.ts`. `--context-report` exit code is propagated in run-bin (`fix(review)`).
- **P8 (#63) spec & plan authoring — slices 1–7 (branch otto/63; full detail in `.otto/memory/*-63-*.json`).** Burned down RUBRIC-FIRST. `plan-rubric.ts` `scorePlanQuality(doc)` → 8 structural criteria (problem, decisions, scopeGuard, fileMap, taskBreakdown, testFirst, verifyCommands, successCriteria) + `ratio`/`missing` + `formatPlanRubric`. `eval.ts` `EvalSignals.planQualityRatio` (set via `scoreTrajectory(…,{planScore})`, kept pure) + a "Plan quality" compare column. `plan-report-cli.ts` `otto-afk --plan-report` scores `.otto/tasks/*/{spec,plan}.md`. `STAGES.plan` + authoring-only `templates/plan.md` (writes only the spec/plan, gate-compatible). `--plan` one-shot (afk-only `cfg.planStage`) REPLACES the chain — do NOT prepend the plan stage, its `NO MORE TASKS` gate sentinel would exit the loop. `plan-checkpoint.ts` pure approve/edit/reject (AFK auto-approves, reject is the safe default). `plan-gate.ts` PASS/FAIL vs `DEFAULT_PLAN_QUALITY_THRESHOLD` 0.75 + re-plan shortfall, wired into `--plan-report`. **GOTCHA: `.otto/LEARNINGS.md` is `@include`d/cat'd into EVERY template; `scripts/smoke-templates.mjs` FAILS any rendered template >20k tokens — keep LEARNINGS terse, put detail in `.otto/memory/` (NOT injected).** Follow-ups: live interactive checkpoint, auto re-plan loop, OTTO_PLAN env, ghafk plan flow.
- **P9 (#64) human-legible run reports — end-to-end thin vertical (branch feat/p9-human-legible-reports; detail in `.otto/memory/*-64-*.json`).** (1) `templates/quality-report.md` REORDERED layperson-first: Verdict → What Changed/Why/How To Verify/What To Watch/What I Was Unsure About (plain prose, no SHAs) → `_Engineer detail below_` divider → Task Source/Evidence/Checklist/Gaps. H1 `# Otto quality report` is STABLE — it's the persistence marker. (2) `run-report.ts` `writeRunReport`/`readRunReport` (best-effort, never throw) + `hasRunReport(result)` (keys on the H1, not stage name — the implementer emits the report; afk/plan-mode emits none) + `report` artifact kind. `loop.ts` tracks the latest report-bearing `sr.result`, `finalizeManifest` writes `.otto/runs/<id>/report.md` + links it via `collectArtifacts` (existsSync, like review-followups). (3) NEW `report-explain.ts` pure `formatPlainReport(manifest, reportText|null)` (report prose first, then a run-facts footer; null → absent-note + facts) + `runExplain` mirroring `runInspect`; NEW `otto-explain` bin. (4) `report-rubric.ts` mirrors `plan-rubric.ts` exactly: `scoreReportLegibility(doc)` checks 7 structural P9 criteria (verdict, whatChanged, why, howToVerify+numbered step, whatToWatch, uncertainty, engineerDivider); `EvalSignals.reportLegibilityRatio` added to `eval.ts` via `opts.reportScore?`, INERT until a caller passes it. **GOTCHA: `scripts/quality-report-samples.test.mjs` parses the contract's `## ` headings AND asserts every sample in `docs/quality-report-samples.md` carries all of them — changing contract sections REQUIRES updating both the test's expected list and all 4 samples.** Deferred: afk report coverage (`prompt.md` doesn't `@include` the contract), embedded before/after evidence (persist diff/screenshots), `otto-inspect --plain` alias.
- **P10 (#65) live execution visualization — all three sub-features (branch feat/p10-live-execution-visualization; detail in `.otto/memory/*-65-*.json`).** ONE shared pure `run-view.ts` `RunView` + `buildRunView(manifest,stages,opts?)` + `formatDoneCard`/`formatLiveTree` is the spine — loop done-card, `otto-tail`, and the console all render from it; ALL styling via `stream-render.ts` primitives so `NO_COLOR`/non-TTY degrade free. **GOTCHA: `formatDoneCard`'s FIRST line MUST keep the greppable `Otto <reason> · N iteration(s) · $<cost>` substring (loop.test.ts greps it).** `plan-progress.ts` pure checkbox parser (returns FRESH zero-state, never a shared ref). `next-action.ts` NEW shared module — `NEXT_ACTION`+`nextActionFor` extracted from `loop.ts` to break the `loop↔run-view` cycle (run-view→next-action, loop re-exports). `console-ui.ts` `EventSink {setStage,onEvent}`: `VerboseSink` delegates to `renderEvent` (today's firehose), quiet `ConsoleUi` classifies edit/commit/test/error + SUPPRESSES assistant text & Read/Glob/Grep; `runner.ts:711` now `(options.sink ?? new VerboseSink()).onEvent(parsed)` (no-sink = byte-identical to before; codex/parse/settle untouched), threaded loop→`stage-exec`→`runStage`; loop builds ONE sink/run, QUIET by default, `--verbose` (cli-help `flags.verbose`→`runLoop({verbose})`) restores firehose. `tail.ts` `runTail` mirrors `runInspect`, polls the bundle (injectable `sleep`/`maxPolls`/`readManifest` → deterministic, dual-exit on `finishedAt`|cap), live-tree while running / done-card when finalized; NEW `otto-tail` bin (`otto-watch` stays the ghafk daemon). Deferred: TUI redraw, HTML/markdown live page, persisted per-action event log, router decision in console.
