...
- [x] **1. Skill substrate `skills.ts`** — `Skill` type (`name`, `version`,
      ...
      `skills.test.ts`.
- [x] **2. Validation governance** — pure `skillStatus(skill, now)` →
...
- [x] **3. Retrieval `selectSkills`** — pure `selectSkills(skills, {assessment,
...
- [x] **4. Candidate identification `findSkillCandidates`** — pure heuristic over
      ...
      `skills.test.ts`.
- [x] **5. `otto-skills` read-only bin** — `skills-cli.ts`: pure
...
- [x] **6. Run-report + eval surfacing** — optional `skillsUsed?: SkillUsage[]`
...
- [x] **7. Docs** — README (Why Otto skills bullet + `otto-skills` in How-it-works),
