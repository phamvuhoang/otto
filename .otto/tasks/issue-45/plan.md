...
- [x] **1. `otto-runs list`** — pure `formatRunsList(summaries)` + thin
...
- [x] **2. `otto-eval compare <run-a> <run-b>`** — a read-only, non-paid `compare`
...
- [x] **3. `--explain-routing`** — pure `explainRouting(route, decision?)` formatter
      ...
      `loop.test.ts`.
- [x] **4. Docs** — README (Why Otto operator bullet + the new commands in the
