...
Guidance for Codex (and other agent CLIs) working in this repo. Behavioral rules: [.claude/CLAUDE.md](.claude/CLAUDE.md).
...
| Tools & context    | `tools.ts`, `tools-cli.ts`, `context-budget.ts`, `context-report.ts`, `context-compressor.ts`, `headroom-adapter.ts` | `.otto/tools/<name>.json` typed adapters under policy scope; `otto-tools list/why/health`. `--context-compressor headroom` runs Headroom's **local** `compress()` library over retrievable `@spill` output (issue bodies — never the diff under review) via a `python3` bridge, kept only when extracted anchors survive (`OTTO_HEADROOM_BIN` = command-mode escape hatch) — reversible, measured, and **governed** by the registered `.otto/tools/headroom.json` + `.otto/policy.json` (`authorizeCompressor`, at the render boundary — not stage-gated); `--context-report`/`--token-mode` measure budget. Personal MCP/plugin config is never inherited. |
