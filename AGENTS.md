...
| Tools & context    | `tools.ts`, `tools-cli.ts`, `context-budget.ts`, `context-report.ts`, `context-compressor.ts`, `headroom-adapter.ts` | `.otto/tools/<name>.json` typed adapters under policy scope; `otto-tools list/why/health`. `--context-compressor headroom` runs Headroom's **local** `compress()` library over `@spill` output via a `python3` bridge (`OTTO_HEADROOM_BIN` = command-mode escape hatch) — reversible, measured, and **governed** by the registered `.otto/tools/headroom.json` + `.otto/policy.json` (`authorizeCompressor`, at the render boundary — not stage-gated); `--context-report`/`--token-mode` measure budget. Personal MCP/plugin config is never inherited. |
