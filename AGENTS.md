...
Otto is a Node/TypeScript harness that drives the Codex CLI against a target repository in an iterating implementer → reviewer loop, running `Codex` directly on the host. It ships as a pnpm monorepo with two npm packages:

- `@phamvuhoang/otto-core` (`packages/core`) — library: loop driver, native-sandbox runner, template renderer, stage registry. ESM, TS-compiled to `dist/`.
- `@phamvuhoang/otto` (`apps/cli`) — CLI exposing `otto-afk` (plan/PRD loop) and `otto-ghafk` (GitHub-issue loop) bin entries. Hand-written JS bins, no build step. Depends on `@phamvuhoang/otto-core` via `workspace:^`.
...
Per-package: `pnpm --filter @phamvuhoang/otto-core build` (only core has a build).
...
(cd apps/cli      && pnpm pack --pack-destination /tmp/otto-packs)
npm i -g /tmp/otto-packs/phamvuhoang-otto-core-*.tgz /tmp/otto-packs/phamvuhoang-otto-*.tgz
otto-afk          # → prints usage
```
...
```bash
otto-afk "<plan-and-prd>" <iterations>          # plan/PRD-driven loop
otto-ghafk <iterations>                          # GitHub-issue-driven loop
otto-afk --print-config                          # diagnose: print workspace / runner / sandbox config
```

Bins also accept `--help` / `-h`. `$OTTO_WORKSPACE` overrides cwd as the target workspace; `$OTTO_RUNNER` selects `sandbox` (default — native OS sandbox, writes confined to the workspace) or `host` (unsandboxed); `$OTTO_SANDBOX_NET` is an optional comma-separated egress domain allowlist for the sandbox. Other env knobs: `$OTTO_RESULT_GRACE_MS` (post-result grace timer, default `30000`, `0` disables), `$OTTO_MODEL` (pass-through `--model <value>` to the Codex CLI; unset = CLI default), `$NO_COLOR` / `$TERM=dumb` (disable ANSI). Bins also accept `--version`/`-V`, `--no-keep-alive`, `--max-retries <N>`, `--detach`, `--log <path>`, `--notify` (see README "Running AFK"). Docker is no longer required. npm releases are automated via release-please — see [RELEASING.md](RELEASING.md).
...
4. **`runner.ts`** (`runStage`) — host runner plumbing.
   - `runStage`: writes the rendered prompt to `<workspaceDir>/.otto-tmp/.run-<pid>-<iter>-<ts>.md`, spawns `Codex --verbose --print --output-format stream-json --permission-mode <mode> "Read the full instructions from ./.otto-tmp/<file> …"` with `cwd = workspaceDir`. When `OTTO_RUNNER=sandbox` (default), writes a transient `--settings` JSON enabling the native OS sandbox with writes confined to the workspace. Streams NDJSON from stdout, captures the `result` event's payload as the stage return value. Tempfiles cleaned in `finally`.
...
| `@spill[?]:<name>=…` | Run a command, write its stdout to `<spill-dir>/<name>`, and substitute the workspace-relative path `./.otto-tmp/spill-…/<name>` into the prompt (the agent `Read`s it). The `?` form writes a fallback string on non-zero exit; `<name>` must be a plain filename (no path separators / `..`). Keeps large outputs (HEAD patch in `review.md`, full issue bodies in `ghafk.md`) out of the prompt. Requires `spillHostDir`/`spillRefPath` (supplied per-stage by `runLoop`). |
...
Every run writes to `<workspaceDir>/.otto-tmp/` on the host (gitignored): the rendered prompt as `.run-<pid>-<iter>-<ts>.md` (cleaned in `finally`, may leak on SIGKILL — safe to delete) a per-stage spill dir `spill-<pid>-<iter>-<stageIdx>-<ts>/` holding `@spill` output (also cleaned in `finally`), and the NDJSON stream log as `logs/<ts>-iter<N>-<stageName>.ndjson` (kept; `--detach` adds `logs/detached-<pid>.log`).
...
- **No build step for `apps/cli`.** Bins are hand-written JS that `import { runAfk } from "@phamvuhoang/otto-core"`. Don't add TS to `apps/cli` — keep the bin layer flat.
