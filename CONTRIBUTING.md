...
If you just want to _run_ Otto against your own repo, see [`./README.md`](./README.md)
...
pnpm --filter @phamvuhoang/otto-core build   # core only
pnpm -r clean                 # wipe dist/ + tsconfig.tsbuildinfo
```

```powershell
pnpm -r build
pnpm --filter @phamvuhoang/otto-core build
pnpm -r clean
```
...
```
packages/core/          @phamvuhoang/otto-core (library; the only built package)
  src/                  12 TS modules + __tests__/  (see docs/ARCHITECTURE.md)
  templates/            prompt.md, ghprompt.md, afk.md, ghafk.md, review.md
  dist/                 tsc output (gitignored)
apps/cli/               @phamvuhoang/otto (hand-written JS bins; no build)
  bin/                  otto-afk.js, otto-ghafk.js
...
3. **Wire it into a chain** in [`main.ts`](./packages/core/src/main.ts) (the
   `otto-afk` chain) and/or [`gh-main.ts`](./packages/core/src/gh-main.ts) (the
   `otto-ghafk` chain), via the `stages:` array passed to `runLoop`.
...
- [`packages/core/templates/prompt.md`](./packages/core/templates/prompt.md) — the
  `otto-afk` (plan/PRD) playbook: where the work comes from (`<inputs>`) + progress recording,
...
- [`packages/core/templates/ghprompt.md`](./packages/core/templates/ghprompt.md) — the
  `otto-ghafk` (GitHub-issue) playbook: issue triage + close/comment the issue, plus the same
...
npm i -g /tmp/otto-packs/phamvuhoang-otto-core-*.tgz \
         /tmp/otto-packs/phamvuhoang-otto-*.tgz
otto-afk          # → prints usage
```

```powershell
pnpm -r build
pnpm --filter @phamvuhoang/otto-core pack --pack-destination $env:TEMP\otto-packs
pnpm --filter @phamvuhoang/otto      pack --pack-destination $env:TEMP\otto-packs
npm i -g (Get-ChildItem $env:TEMP\otto-packs\phamvuhoang-otto-core-*.tgz).FullName `
         (Get-ChildItem $env:TEMP\otto-packs\phamvuhoang-otto-*.tgz).FullName
otto-afk          # -> prints usage
```

`pnpm pack` rewrites the CLI's `workspace:^` core dependency to a concrete spec in
the tarball, so the installed `@phamvuhoang/otto` resolves a real `@phamvuhoang/otto-core`.
...
The repo ships two independently versioned npm components: `@phamvuhoang/otto-core`
and `@phamvuhoang/otto`. Flow:
...
4. The tag triggers publishing:
   - `otto-core-v*` / `otto-v*` → `publish-npm.yml` (publishes to npm; rewrites the
...
- **Templates ship in the core tarball** (`@phamvuhoang/otto-core` `files: ["dist",
