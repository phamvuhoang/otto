# Releasing Otto

> This document is the single source of truth for how Otto's npm packages are
> released. It supersedes `docs/PUBLISHING.md`.

Otto ships two independently versioned npm packages (Docker is no longer required):

| Component    | Path            | Artifact                  | Tag schema          |
| ------------ | --------------- | ------------------------- | ------------------- |
| `otto-core` | `packages/core` | npm `@phamvuhoang/otto-core` | `otto-core-vX.Y.Z` |
| `otto`      | `apps/cli`      | npm `@phamvuhoang/otto`      | `otto-vX.Y.Z`      |

Releases are driven by [release-please](https://github.com/googleapis/release-please)
in manifest mode. The config lives at the repo root in `release-please-config.json`
and `.release-please-manifest.json` (the manifest is the source of truth for the
last-released version of each component). Conventional-commit history on `main`
determines what gets released; you write `feat:` / `fix:` commits as usual and a
Release PR appears.

> **release-please owns the version state — never hand-edit it.**
> `.release-please-manifest.json` and the per-package `version` fields in
> `packages/core/package.json` / `apps/cli/package.json` are written by
> release-please, not by hand. A manual bump (commit `ea1c006`, "bump to 0.6.3")
> once desynced the manifest and set off a cascade of phantom versions (0.6.0,
> 0.6.3) and revert churn. Let the Release PR carry the bump; to force a specific
> version, use a `Release-As:` footer (§5) — never edit those files directly.

## 1. Current versions

<!-- status-table:start -->
| Component | Artifact | Version | Released | Tag |
| --- | --- | --- | --- | --- |
| `otto-core` | npm `@phamvuhoang/otto-core` | `0.35.0` | 2026-07-18 | [`otto-core-v0.35.0`](https://github.com/phamvuhoang/otto/releases/tag/otto-core-v0.35.0) |
| `otto` | npm `@phamvuhoang/otto` | `0.10.11` | 2026-07-18 | [`otto-v0.10.11`](https://github.com/phamvuhoang/otto/releases/tag/otto-v0.10.11) |
<!-- status-table:end -->

> The block above is regenerated by `scripts/update-status-table.mjs`. The
> release-please workflow runs it on `main` after each release (so the dates and
> tag links stay current); it can also be run locally for a manual refresh:
>
> ```bash
> node scripts/update-status-table.mjs          # rewrite the block in place
> node scripts/update-status-table.mjs --check   # exit 1 if it would change
> ```
>
> Dates and tag links populate once the first tag for a component exists.

## 2. How to cut a release

1. Land your work on `main` with Conventional Commits (see §3). No version bump by
   hand — release-please computes it.
2. release-please opens (or updates) **a single combined Release PR** covering every
   component that has unreleased commits. The PR bumps each component's version and
   amends its `CHANGELOG.md`. (The RELEASING.md status table is refreshed on `main`
   after the release — never on the Release PR branch, which release-please must own
   exclusively.)

   > **Why one combined PR, not one per component.** `separate-pull-requests` is
   > `false` deliberately. Separate per-component PRs both edit
   > `.release-please-manifest.json`; because its two version lines are adjacent, git
   > cannot 3-way-merge them, so whenever a `packages/core` change cascaded a `otto`
   > dependency bump (via the `node-workspace` plugin) the second PR conflicted on the
   > manifest every time. One combined PR removes the sibling that caused the conflict.
3. Review the PR: confirm the proposed versions and the CHANGELOG diffs read the way
   you want users to see them.

   > **Smoke the published shape first.** Run `node scripts/smoke-pack-install.mjs`
   > on the Release PR branch. It packs both packages, installs the tarballs into an
   > isolated prefix, and asserts the installed `otto-afk` prints usage and resolved
   > config — proving `@phamvuhoang/otto` resolves a real `@phamvuhoang/otto-core`
   > before you publish. (Details: CONTRIBUTING.md § Smoke-test published artifacts.)
4. **Merge the Release PR.** That single action creates each released component's tag
   (`<component>-vX.Y.Z`) and GitHub Release.
5. The tag triggers the matching publish workflow:
   - `otto-core-v*` / `otto-v*` → `publish-npm.yml` → publishes to npm, then
     attaches the `.tgz`, SBOM, and cosign attestation to the Release.
6. Watch the Actions tab; the publish workflow run links to the published artifact.

> **Token requirement.** release-please must create tags with a Personal Access
> Token (`RELEASE_PLEASE_TOKEN` secret, repo scope), because a tag created with the
> default `GITHUB_TOKEN` does **not** trigger the `push: tags` publish workflows.
> Required secrets: `RELEASE_PLEASE_TOKEN`, `NPM_TOKEN`. If only `GITHUB_TOKEN` is
> available, merge still creates the tag but you must start the publish manually
> (`workflow_dispatch` on `publish-npm.yml` with the tag name).

### Release-quality gate

Machine-green is necessary but **not sufficient**. Before merging the Release PR
for a major change (a new run mode, a breaking change, or any `feat:` that shifts
user-visible behavior), require **both**:

1. **Machine verification** — `pnpm -r typecheck && pnpm -r test && pnpm test` all
   green, plus the published-shape smoke (`node scripts/smoke-pack-install.mjs`,
   §2 step 3).
2. **A human-readable quality report** — run the read-only verify gate
   (`otto-afk --verify …`) so it emits the
   [Otto quality report](packages/core/templates/quality-report.md) (verdict +
   evidence + gaps), then read it. The gate clears only when a human accepts the
   report — verdict **Accepted** or **Accepted with follow-ups**. A *Needs human
   review* or *Rejected* verdict does **not** clear it, even with a fully green
   pipeline: tests passing is evidence, not the verdict.

Record the accepted verdict (and any follow-ups) in the Release PR description so
the release carries a human-readable verification trail, not just CI checkmarks.

## 3. Conventional Commit guide

The commit type drives both the version bump and the CHANGELOG section. Convention
is unchanged from the existing history — adopting release-please required no
contributor behavior change.

| Prefix      | Bump (0.x)      | CHANGELOG section         | Triggers a release? |
| ----------- | --------------- | ------------------------- | ------------------- |
| `feat:`     | minor           | Features                  | yes                 |
| `fix:`      | patch           | Bug Fixes                 | yes                 |
| `perf:`     | patch           | Performance Improvements  | yes                 |
| `deps:`     | patch           | Dependencies              | yes                 |
| `refactor:` | patch           | Code Refactoring          | yes                 |
| `chore:`    | —               | Miscellaneous Chores      | no (changelog only) |
| `ci:`       | —               | (hidden)                  | no                  |
| `docs:`     | —               | (hidden)                  | no                  |
| `test:`     | —               | (hidden)                  | no                  |
| `style:`    | —               | (hidden)                  | no                  |

Breaking changes: append `!` (e.g. `feat!:`) or add a `BREAKING CHANGE:` footer. See
§4 for how breaking changes map to versions under this repo's policy.

Which component a commit belongs to is decided by the path it touches:

- `packages/core/**` → `otto-core`
- `apps/cli/**` → `otto`

## 4. Version policy

- **Independent versions per component.** Each component keeps its own version number
  and tag — they do not share a version. They are proposed together in one combined
  Release PR (see §2), but a commit only bumps the component whose path it touches.
  The one cascade: because `apps/cli` depends on `packages/core`, the `node-workspace`
  plugin gives `otto` a dependency-driven patch bump when `otto-core` releases.
- **0.x is treated as a stable contract.** Stricter than the SemVer spec's "0.x means
  anything goes": while on 0.x, a backward-compatible feature is a **minor** bump, a
  bug fix is a **patch** bump, and any **breaking change forces `1.0.0`**. Downstream
  users can pin a 0.x version and trust that a patch or minor upgrade will not break
  them.
- **Enforcing the breaking→1.0.0 rule.** release-please will not auto-jump from 0.x to
  1.0.0 on a breaking change (pre-1.0 it bumps the minor). When a breaking change
  lands, the maintainer forces the major with a `Release-As:` footer (§5):
  `Release-As: 1.0.0`.

## 5. Pre-release tags and version overrides

Pre-release tags (`-rc.N`, `-next.N`) are permitted but off by default. To opt in for
a single release, add a `Release-As:` footer to a commit on `main`; release-please
honors it for the next Release PR of the component that commit touches.

```
feat: gate the new reviewer stage behind a flag

Release-As: 0.3.0-rc.1
```

The same footer is how you force any exceptional version — e.g. the breaking→1.0.0
jump from §4:

```
feat!: remove deprecated flag from runLoop public API

BREAKING CHANGE: the deprecated option is removed with no replacement.
Release-As: 1.0.0
```

## 6. Rollback runbook

Act fast; don't invent a process during an incident.

### npm (`@phamvuhoang/otto-core`, `@phamvuhoang/otto`)

npm does not allow unpublishing after 72h (or once another package depends on the
version). Deprecate the bad version(s) so installs surface a warning, then ship a
fixed version through the normal flow.

```bash
# Deprecate each affected version with a message pointing at the fix.
npm deprecate @phamvuhoang/otto-core@0.1.2 "Broken release — upgrade to 0.1.3"
# A range works too:
npm deprecate "@phamvuhoang/otto@>=0.2.0 <0.2.2" "Broken release — upgrade to 0.2.2"

# Then cut a fixed release the normal way (commit a fix:, merge the Release PR).
# To clear a deprecation after the fix lands:
npm deprecate @phamvuhoang/otto-core@0.1.2 ""
```

### GitHub Release

Edit the affected Release to mark it clearly (prefix the title with `[YANKED]`) and
note the replacement version in the body, so the paper trail stays honest.

## 7. Compatibility matrix

Known-good combinations. Update this when a release changes a compatibility
boundary (e.g. the CLI starts requiring a newer core).

| `@phamvuhoang/otto` (CLI) | requires `@phamvuhoang/otto-core` |
| ---------------------- | ------------------------------ |
| `0.6.0`                | `^0.6.0`                       |
| `0.1.0`                | `^0.1.1`                       |

The CLI declares its core dependency as `workspace:^` in the repo; the publish
pipeline rewrites that to the concrete `^<core-version>` it was built against, so an
installed `@phamvuhoang/otto` always pulls a compatible `@phamvuhoang/otto-core`.

## 8. Tag naming

Each component owns a tag prefix so the publish workflow matches only its own
releases:

| Component    | Tag                 | Workflow filter   |
| ------------ | ------------------- | ----------------- |
| `otto-core` | `otto-core-vX.Y.Z` | `otto-core-v*`   |
| `otto`      | `otto-vX.Y.Z`      | `otto-v*`        |

## 9. Package contents

Each tarball ships only the paths in its `package.json` `files:` array (plus the
npm-implicit `package.json` and `LICENSE`). Keep this list in sync when packaging
changes — `scripts/releasing-contract.test.mjs` fails if it drifts from
`package.json`:

- **`@phamvuhoang/otto-core`** (`packages/core`) ships: `dist`, `templates`, `README.md`.
- **`@phamvuhoang/otto`** (`apps/cli`) ships: `bin`, `scripts`, `README.md`.

`otto-core` builds `dist/` (and ships `templates/`) via `prepublishOnly`; `otto`
is hand-written JS with no build step. The `node scripts/smoke-pack-install.mjs`
check (§2) packs both tarballs and installs them to prove the published shape.
