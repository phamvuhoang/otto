...
| [`pr-review-state.ts`](../packages/core/src/pr-review-state.ts)       | Durable per-composite-identity (`repository, pr, headSha, inputFingerprint`) review state under gitignored `.otto/review-state/github/<owner>/<repo>/<pr>/<head-sha>/<fingerprint>.json`, plus an OS advisory lock (`flock`, via the optional native `fs-ext`) held for the run on a persistent per-identity `<state-path>.lock` file (stable inode). The kernel releases it on process death — automatic crash recovery, no heartbeat or tombstone. `fs-ext` is loaded lazily and only when a review actually acquires a lease (review-only; needs a build toolchain), and advisory `flock` is reliable only on a local filesystem (single-daemon-per-repo).                                                |
...
runPullRequestReview (pr-review.ts)
   readReviewState / acquireReviewLease      composite (repo, pr, headSha, inputFingerprint) dedup + OS flock lease
   ...
   finally: release lease (flock LOCK_UN), cleanup worktree
```
...
[`pr-review-state.ts`](../packages/core/src/pr-review-state.ts) persists one JSON record per composite identity under `.otto/review-state/github/<owner>/<repo>/<pr>/<head-sha>/<fingerprint>.json` (gitignored), plus an OS advisory lock (an exclusive non-blocking `flock`, via the optional native `fs-ext`) taken for the run's lifetime on a persistent per-identity `<state-path>.lock` file, so two runs against the SAME workspace cannot both review one identity — the kernel serializes acquisition, so two racers can never both win. Crash recovery is automatic and needs no heartbeat or tombstone: the kernel drops the `flock` the instant the holder dies, so a later run's exclusive acquire simply succeeds (the lock lives in the kernel keyed to the open fd, not in the file body; the `.lock` file is left persistent so its stable inode keeps all acquirers contending on the same lock). `fs-ext` is a native dependency loaded lazily — only when a review actually acquires a lease — so merely importing the package pulls in nothing; a review host needs a build toolchain (or a prebuilt binary). Because advisory `flock` is unreliable over NFS, this exclusion is scoped to a local filesystem (single daemon per repo). [`pr-review-output.ts`](../packages/core/src/pr-review-output.ts) defines the four idempotency markers a restart reconciles on:
