...
| [`review-main.ts`](../packages/core/src/review-main.ts)               | `runReview` bin entry: parse → resolve config → `--print-config` (local-only preflight, GitHub checks deferred) → the SAME `runReviewRemotePreflight` (viewer/auth + exact-label + origin) for BOTH branches, then either one-shot input resolution + `runPullRequestReview`, or fork into `runPullRequestReviewWatch`. A typed lease/storage failure out of the one-shot path is caught and surfaced as one actionable line (exit 1), never a raw stack.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| [`pr-review-skill.ts`](../packages/core/src/pr-review-skill.ts)       | `resolveReviewSkill` — the built-in `builtin:otto-code-review` default, or one explicit `.otto/skills/<name>/` skill gated on its recorded static-validation class (`afk-safe`/`stage-scoped("review")`, the same gate `routeSkillsForStage` applies) plus a checksum-clean body. Never falls back to the built-in after an explicit request fails.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| [`pr-review-state.ts`](../packages/core/src/pr-review-state.ts)       | Durable per-composite-identity (`repository, pr, headSha, inputFingerprint`) review state under gitignored `.otto/review-state/github/<owner>/<repo>/<pr>/<head-sha>/<fingerprint>.json`, guarded by TWO OS advisory locks (`flock`, via the optional native `fs-ext`): `acquireReviewLease` — a non-blocking exclusive flock on the per-identity `<state-path>.lock` (stable inode), held for the run — and `acquirePublicationLease` — a BLOCKING exclusive flock on a PR-scoped `publication.lock` (keyed only by `repository/pr`), held only across the shared summary comment's reconcile/write. Acquisition order is always composite lease FIRST, publication lease SECOND; the kernel releases either on process death — automatic crash recovery, no heartbeat or tombstone. `writeReviewState` is fail-closed: a durable-write failure raises `ReviewStatePersistenceError` rather than being swallowed, so a run never reports `succeeded` with unpersisted terminal state. `fs-ext` is loaded lazily and only when a review actually acquires a lease (review-only; needs a build toolchain), and advisory `flock` is reliable only on a local filesystem (single-daemon-per-repo). |
...
| [`pr-review-watch.ts`](../packages/core/src/pr-review-watch.ts)       | `otto-review --watch` daemon (entered by `review-main.ts` only after the SAME viewer/auth + exact-label + origin/repository preflight a one-shot run performs, so polling never starts against bad auth/label/origin): polls one repo's open/non-draft/labelled PRs, re-resolves the review input every poll, selects the first runnable composite identity, and drives the SAME one-shot pipeline. `isFatalDaemonError` rethrows a `ReviewLeaseError` (missing/broken `fs-ext`, `ENOTSUP`, …) or `ReviewStatePersistenceError` to the outer failure path — the daemon attempts once, releases keepalive/listeners, notifies once, and EXITS, instead of polling forever; an ordinary transient revision failure logs, backs off one interval, and continues. Local to one workspace — run one `--watch` daemon per repository.                                                                                                                                                                                                                                                                                                                                                                 |
...
| [`pr-review.ts`](../packages/core/src/pr-review.ts)                   | `runPullRequestReview` — the pipeline: input artifact → governed skill → isolated worktree/diff → optional PR-body-only compression (never review-input) → read-only lens/verify panel (`access:"read-only"`, `permissionMode:"plan"`) → `analysis.json` → re-query reconcile → canonical output/publication. `readReviewAnalysisArtifact` strictly re-validates a persisted `analysis.json` (schema `v2`, identity/path/diff-SHA-256 integrity, run-root/same-inode/`O_NOFOLLOW` reads) before any resume — invalid evidence is treated as absent and a fresh analysis runs. `currentRequiredSinks`/`currentSinksComplete` make a prior `succeeded` state terminal only when EVERY sink the CURRENT invocation requests (`config.output` + `config.githubReview`) already has a receipt; a missing sink resumes from validated analysis at zero additional model cost.                                                                                                                                                                                                                                                                                                                         |
...
runPullRequestReview (pr-review.ts)
   readReviewState / acquireReviewLease      composite (repo, pr, headSha, inputFingerprint) dedup + non-blocking OS flock lease
   re-read state UNDER the lease; readReviewAnalysisArtifact strictly re-validates any persisted analysis.json (schema v2, diff SHA-256, path/inode checks) before resume — invalid → fresh analysis
   currentSinksComplete(config.output, config.githubReview)  a prior succeeded run is terminal only if EVERY current sink already has a receipt; a missing sink resumes at $0
   ...
   renderCanonicalReview → text | markdown | [comment] acquirePublicationLease (BLOCKING, PR-scoped) → upsertSummaryComment (marker-idempotent) → release publication lease
   [--github-review] mapFindingsToDiff → publishFormalReview (marker-idempotent, deterministic APPROVE/COMMENT/REQUEST_CHANGES) — composite lease only, no publication lock
   writeReviewState                            terminal status persisted for next-run dedup; a durable-write failure raises ReviewStatePersistenceError (never a false "succeeded")
   finally: release composite lease (flock LOCK_UN), cleanup worktree
```
...
[`pr-review-state.ts`](../packages/core/src/pr-review-state.ts) persists one JSON record per composite identity under `.otto/review-state/github/<owner>/<repo>/<pr>/<head-sha>/<fingerprint>.json` (gitignored), guarded by TWO persistent, stable-inode OS advisory locks (`flock`, via the optional native `fs-ext`), always acquired in the SAME fixed order:

1. `acquireReviewLease` — a non-blocking exclusive flock on the per-composite-identity `<state-path>.lock`, held for the run's lifetime, so two runs against the SAME workspace cannot both review one identity. A busy composite identity returns `{ acquired: false, reason: "busy" }` and the caller `skip`s with no work.
2. `acquirePublicationLease` — a BLOCKING exclusive flock on a PR-scoped `publication.lock` (keyed only by `repository/pr`, the SAME file across every head SHA/fingerprint of that PR), acquired ONLY around the shared summary comment's list→create/update and released immediately after — never held across the independent formal-review write. It serializes the ONE mutable summary comment across DISTINCT composite identities (different input fingerprints) that all legitimately publish to it; the fixed composite-first/publication-second order rules out deadlock by construction.

The kernel serializes each lock's acquisition — two racers can never both win — and crash recovery is automatic for either lock, needing no heartbeat or tombstone: the kernel drops a `flock` the instant its holder dies, so a later exclusive acquire simply succeeds (each lock lives in the kernel keyed to the open fd, not in the file body; both lock files are left persistent so their stable inodes keep all acquirers contending on the same lock). `fs-ext` is a native dependency loaded lazily — only when a review actually acquires a lease — so merely importing the package pulls in nothing; a review host needs a build toolchain (or a prebuilt binary). Because advisory `flock` is unreliable over NFS, this exclusion is scoped to a local filesystem (single daemon per repo). `writeReviewState` is FAIL-CLOSED: a durable-write failure (full disk, unwritable run dir, EIO) raises a typed `ReviewStatePersistenceError` (carrying the state path) instead of being swallowed, so a run never reports `succeeded` — or proceeds to a further remote write — with unpersisted terminal state; any receipt already proven remotely is finalized into local evidence first. [`pr-review-output.ts`](../packages/core/src/pr-review-output.ts) defines the four idempotency markers a restart reconciles on:
...
Every reserved `<!-- otto-review` marker prefix a model authors anywhere in a finding/title/etc. is HTML-escaped before rendering, so model-controlled text can never forge a syntactically valid marker; canonical-envelope parsing requires each marker at its documented fixed position, occurring EXACTLY once, before recovery ever trusts a remote body.

[`pr-review-publish.ts`](../packages/core/src/pr-review-publish.ts) re-queries the PR immediately before every remote write (`reconcilePublication`) — a moved head is `superseded`, a closed/draft/unlabelled PR is `cancelled`, and neither ever produces a stale comment or review — then create-or-updates the summary comment / formal review by marker rather than a locally-remembered id, so a restart with lost local state finds and reuses its own prior write instead of duplicating it. GitHub refusing a bot's self-approval is a permanent (non-retryable) `publish-failed`; a transient failure gets a bounded exponential backoff (`nextPublicationRetryAt`: 60s → 120s → 240s… capped at 15 minutes). Before either resume or lost-state recovery trusts a persisted `analysis.json` (schema `v2`) or a remote body, `readReviewAnalysisArtifact`/the canonical-envelope parsers strictly validate it — identity/enum/shape, the diff artifact's SHA-256 digest, and run-root/same-inode/`O_NOFOLLOW` checks on every artifact path — and treat anything that fails as absent, running a fresh analysis rather than publishing unverified content. A prior `succeeded` state is terminal only when every sink the CURRENT invocation requests (`config.output` plus, when set, `config.githubReview`) already has a receipt; a success missing a now-requested sink resumes just that sink from the validated analysis at zero additional model cost (`currentRequiredSinks`/`currentSinksComplete` in `pr-review.ts`).
