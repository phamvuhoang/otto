...
| `OTTO_INCLUDE_SUB_ISSUES`   | _(off)_                           | When truthy (`1`/`true`/`yes`), expand `--issue` into its open sub-issues (native GitHub sub-issues, or a markdown `- [ ] #N` task-list fallback), depth-first, parent skipped (`otto-ghafk`; requires `--issue`). Equivalent to `--include-sub-issues`. |
...
- **`sandbox` (default)** — Filesystem **writes are confined to the workspace**. Claude receives a transient `--settings` JSON that enables its native OS sandbox (Seatbelt on macOS; `bubblewrap`+`socat` on Linux). Codex receives global `--ask-for-approval never` plus `exec --ignore-user-config --sandbox workspace-write`. `OTTO_SANDBOX_NET` applies to Claude's native sandbox settings; Codex owns its own network behavior.
- **`host`** — The active runtime runs unsandboxed with full host access. Claude omits the sandbox settings file; Codex receives `exec --ignore-user-config --sandbox danger-full-access`. Only safe when the workspace is a git-recoverable, throwaway tree.

Claude stages run with `--permission-mode bypassPermissions`; Codex stages run with global `--ask-for-approval never` and `exec --ignore-user-config`. AFK is non-interactive, so bash/edit approval must be automatic, and personal Codex config such as MCP servers/plugins should not be loaded into unattended stages. The sandbox runner and the git-recoverable workspace are what bound the blast radius. See **[SECURITY.md](../SECURITY.md)** for the full threat model.
...
With no `checks` key, every attestation seam short-circuits and runs behave byte-for-byte as before.

**Where they run.** After a commit by the single `reviewer`, the review panel's `review-synth`, or `apply-review-implementer`. In `--verify` mode, matrix rows whose `method` is `test` and whose cited command **exactly matches** a configured check are re-executed rather than merely checked for artifact existence.

**Order matters — the ladder is fail-fast.** Commands run top to bottom and stop at the first non-zero exit, so a red `pnpm -r typecheck` never pays for the slow suite. Unrun commands are reported as `skipped`, never as passes. Put your cheapest check first.

**What it changes.** A run whose **final** attestation is red exits with reason `done with failing checks` and scores `succeeded: false` in eval, and the run report gains an **Attested Checks** section flagging the disagreement — the agent committed a fix, so it claimed green; the harness observed otherwise.

A failure an earlier iteration hit and a later one fixed does **not** sink the run: that is the review loop working. It is recorded as churn evidence (`everFailed`) and shown as "recovered", while the verdict follows the terminal state.

**Safety.** Every command is checked against `.otto/policy.json` before it is spawned; a blocked command is recorded as a failure and never executed. Each command has a 10-minute timeout. Agent-emitted matrix commands are matched exactly against your configured list — never fuzzily — so untrusted text cannot reach a shell.

Inspect the results with `otto-inspect <run-id>`, which renders the run tally and a per-stage `check: PASS|FAIL <cmd> (exit N, Tms)` line.
