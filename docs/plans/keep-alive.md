# Plan: Keep Otto Alive While AFK

> Source PRD: [docs/prd/keep-alive.md](../prd/keep-alive.md) · GitHub issue [#17](https://github.com/phamvuhoang/otto/issues/17)
...
- **Flag surface** (additive on both `otto-afk` and `otto-ghafk`):
...
- **Detach log default** — `.otto-tmp/logs/detached-<pid>.log` (co-located with existing NDJSON logs).
...
- [ ] `vitest` is wired into `packages/core` (`devDependencies`, `test` script). `pnpm --filter @phamvuhoang/otto-core test` runs the suite green.
...
- [ ] Windows smoke: a short `otto-afk` run holds a `SYSTEM` request visible in `powercfg /requests`, released on exit.
...
- [ ] `--detach` is wired on both `otto-afk` and `otto-ghafk`. The wake-lock + retry behavior from Phases 1 and 2 still applies in the detached child.
- [ ] `--log <path>` is honored when present; default is `.otto-tmp/logs/detached-<pid>.log` when absent.
...
- [ ] Smoke: `otto-afk --detach "<plan>" 3` returns the prompt immediately; closing the parent terminal does not kill the loop; `tail -f` on the log path from a fresh shell shows iteration progress.
...
- [ ] `README.md` has a "Running AFK" section linking to `docs/keep-alive.md` and showing the canonical overnight-run recipe (e.g. `otto-afk --detach --notify "<plan>" 50`).
