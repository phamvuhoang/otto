...
- **Mise cache volume**: named docker volume `otto-mise-cache` mounted at `/home/agent/.local/share/mise`. Auto-created on first `docker run -v`. User-overridable via `OTTO_MISE_VOLUME`.
...
- **Env var contract (host → container)**:
  - `OTTO_NODE` — optional. Exact version, major, or alias (`"lts"`). Unset means use baked default.
  - `OTTO_DOTNET` — optional. Exact version or `major.minor`. Unset means use baked default.
  - `OTTO_MISE_VOLUME` — optional. Override the cache volume name.
- **Detection module contract**: `detectVersions(workspaceDir: string) → { node?: string; dotnet?: string; sources: { node?: ManifestSource; dotnet?: ManifestSource }; warnings: string[] }`. Pure, no spawn, no network. Lives in `@phamvuhoang/otto-core`. Called exactly once per `runLoop` invocation, before the iteration loop.
...
No host-side detection yet — the user supplies `OTTO_NODE` / `OTTO_DOTNET` manually. This proves the whole pipeline (image → shim → activation) end-to-end without coupling it to the parser work in phases 2–3.

### Acceptance criteria

- [ ] New Dockerfile bakes mise + Node 20 + Node 22 to `/opt/mise-baked`, plus .NET SDK 8, 9, 10 via apt. Builds clean locally on amd64.
- [ ] Entrypoint shim reads `OTTO_NODE` / `OTTO_DOTNET`, activates Node via mise (using the baked path), validates the requested dotnet SDK is present (warns and falls back otherwise), then `exec`s the claude CLI.
- [ ] Runner.ts forwards `OTTO_NODE` / `OTTO_DOTNET` from `process.env` into the container when set. No volume mount yet.
- [ ] CI workflow publishes a multi-arch (`linux/amd64,linux/arm64`) image as `:v2-alpha` on `workflow_dispatch`. `:latest` is untouched.
- [ ] Manual smoke: `docker run --rm -e OTTO_NODE=20 phamvuhoang/otto-sandbox:v2-alpha node --version` prints `v20.x.x`.
- [ ] Manual smoke: `docker run --rm -e OTTO_DOTNET=8.0 phamvuhoang/otto-sandbox:v2-alpha dotnet --list-sdks` lists 8.0, 9.0, 10.0.
- [ ] Manual smoke: `OTTO_NODE=20 OTTO_IMAGE=phamvuhoang/otto-sandbox:v2-alpha otto-afk --print-config` shows the correct image ref; a real iteration against a Node-20 toy repo completes one round.
...
- [ ] `detect` module exists in `@phamvuhoang/otto-core` with the interface defined in Architectural Decisions. Implemented parsers: `.nvmrc` and `global.json`. All other manifest sources return undefined.
...
- [ ] vitest is added to `packages/core` devDependencies; `pnpm --filter @phamvuhoang/otto-core test` runs and passes. Vitest is excluded from the npm tarball's `files` array.
...
- [ ] Runner.ts mounts `${OTTO_MISE_VOLUME ?? "otto-mise-cache"}:/home/agent/.local/share/mise` on every stage invocation. No volume is mounted when both `OTTO_NODE` and `OTTO_DOTNET` are unset (optional optimization — acceptable to always mount).
- [ ] Entrypoint shim, on each run: if the volume mountpoint is empty, copy/seed from `/opt/mise-baked`. Then activate `OTTO_NODE`; if not present, `mise install node@$OTTO_NODE && mise use --global node@$OTTO_NODE`. Failures surface to stderr and exit non-zero.
- [ ] `OTTO_DOTNET` continues to require a baked SDK; no lazy install for .NET (apt-only, root-required). The shim's existing warn-and-fall-back behavior from phase 1 is preserved.
- [ ] Manual smoke: `OTTO_NODE=24 otto-afk` (no `.nvmrc`, no manifest) on a clean machine — first iteration lazy-installs Node 24 inside the volume, second iteration is fast (no re-install).
- [ ] Manual smoke: `docker volume rm otto-mise-cache && otto-afk` works — seed-from-bake path triggers; Node 20 / 22 still available without download.
- [ ] Manual smoke: `OTTO_MISE_VOLUME=my-proj otto-afk` uses a project-scoped volume; the default volume is unaffected.
- [ ] `pnpm -r typecheck` clean.
...
Operationalize the rollout. After a soak period on `:v2-alpha`, retag to `:v2`, then promote `:v2` to also tag `:latest`. Expose a `runtime_tag` workflow input so baked-version bumps can be cut independently of CLI version bumps. Document the runtime-detection behavior in the README. Bump `@phamvuhoang/otto-core` minor version.
...
- [ ] `@phamvuhoang/otto-core` `package.json` minor version bumped. CLI package `package.json` updated to depend on the new core version. `pnpm publish-all` dry-run is clean.
