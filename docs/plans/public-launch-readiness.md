...
- **Tag schema** (unchanged, source of truth for what to publish): `otto-core-vX.Y.Z` → `@phamvuhoang/otto-core`, `otto-vX.Y.Z` → `@phamvuhoang/otto`, `otto-sandbox-vX.Y.Z` → the sandbox image (`tag=vX.Y.Z`).
...
- [ ] `publish-npm.yml` dispatched for `otto-core-v0.6.3` and for `otto-v0.6.3`; both runs succeed.
- [ ] `publish-image.yml` dispatched with `tag=v0.2.3`, `also_latest=true`, `release_tag=otto-sandbox-v0.2.3`; run succeeds.
- [ ] `npm view @phamvuhoang/otto version` and `npm view @phamvuhoang/otto-core version` both return `0.6.3`.
- [ ] `docker.io/phamvuhoang/otto-sandbox:0.2.3` exists and `:latest` resolves to it.
