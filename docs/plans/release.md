...
- **Components** (three, independently versioned, never lockstep):
  - `otto-core` rooted at `packages/core`
  - `otto` rooted at `apps/cli`
  - `otto-sandbox` rooted at `packages/core`, path-scoped to `packages/core/Dockerfile` and `packages/core/templates/**` (synthetic image component)
- **Tag schema**: `otto-core-vX.Y.Z`, `otto-vX.Y.Z`, `otto-sandbox-vX.Y.Z`. Each publish workflow filters on its own tag prefix.
- **Bootstrap seeds**: `otto-core@0.1.1`, `otto@0.1.0`, `otto-sandbox@0.1.1` (matches the existing `image-v0.1.1` tag). First release PR proposes patch bumps from these baselines based on commits since the last release.
...
- **Existing artifacts are preserved**: `@phamvuhoang/otto-core@0.1.1`, `@phamvuhoang/otto@0.1.0`, and the `image-v0.1.1` Git tag remain. Nothing is re-published or moved.

---

## Phase 1: Bootstrap release-please + rewire `otto-core` publish

**User stories**: 1, 2, 3, 4, 10, 11, 19, 20, 21, 22, 24

### What to build

Adopt release-please for `otto-core` only as the first vertical slice. End-to-end this means: a maintainer pushes a `feat:` or `fix:` commit to `main`, release-please opens a Release PR proposing the next `otto-core` version and the `CHANGELOG.md` diff, the maintainer merges the PR, a `otto-core-vX.Y.Z` tag is created, and the rewired npm publish workflow fires and publishes `@phamvuhoang/otto-core` to the registry.
...
- [ ] `release-please-config.json` and `.release-please-manifest.json` exist at repo root, declaring `otto-core` only, seeded at `0.1.1`
...
- [ ] `.github/workflows/publish-npm.yml` is rewritten to trigger on `push: tags: ['otto-core-v*']` and publishes `packages/core` based on the tag prefix
...
- [ ] Merging the release PR creates a `otto-core-vX.Y.Z` tag, the publish workflow fires, and `@phamvuhoang/otto-core@X.Y.Z` appears on the npm registry
...
## Phase 2: Add `otto` CLI as second release-please component

**User stories**: 5

### What to build

Extend release-please to manage `apps/cli`. The CLI currently lags `otto-core` silently because no CI path publishes it. After this slice, a commit that touches `apps/cli/**` triggers its own Release PR section and tag, and merging it publishes `@phamvuhoang/otto` to npm via the same workflow used for core.

End-to-end: the existing `publish-npm.yml` learns to route by tag prefix (`otto-core-v*` → `packages/core`, `otto-v*` → `apps/cli`). Manifest seeds `otto@0.1.0`. Release-please config gains the second component declaration. The CHANGELOG starts receiving CLI entries in its own subsection.
...
- [ ] `release-please-config.json` declares `otto` rooted at `apps/cli`
- [ ] `.release-please-manifest.json` seeds `otto@0.1.0`
- [ ] `publish-npm.yml` triggers on both `otto-core-v*` and `otto-v*` and routes the publish step to the correct package directory based on `${GITHUB_REF_NAME}`
- [ ] A commit touching only `apps/cli/**` causes release-please to propose a bump for `otto` and not `otto-core`
- [ ] Merging a `otto` release PR creates a `otto-vX.Y.Z` tag and publishes `@phamvuhoang/otto@X.Y.Z` to npm
- [ ] CHANGELOG entries for the CLI appear under a clearly distinct subsection

---

## Phase 3: Add `otto-sandbox` synthetic image component

**User stories**: 6, 12

### What to build

Introduce the third release-please component: a synthetic `otto-sandbox` rooted at `packages/core` but path-scoped to `packages/core/Dockerfile` and `packages/core/templates/**`. This is the novel piece of the configuration: it lets a Dockerfile or template change bump the image version without dragging `otto-core` along, and vice versa.

End-to-end: a Dockerfile or template change on `main` causes release-please to open (or update) a `otto-sandbox` release PR. Merging it creates a `otto-sandbox-vX.Y.Z` tag. The rewritten `publish-image.yml` triggers on that tag, builds the image, pushes to Docker Hub, captures the `sha256:…` digest emitted by buildx, and appends `Image: docker.io/phamvuhoang/otto-sandbox@sha256:…` to the GH Release body via `gh release edit`.
...
- [ ] `release-please-config.json` declares `otto-sandbox` rooted at `packages/core` with the path scope restricted to `Dockerfile` and `templates/**`
- [ ] `.release-please-manifest.json` seeds `otto-sandbox@0.1.1`
- [ ] A commit touching `packages/core/src/**` does **not** open a `otto-sandbox` release PR
- [ ] A commit touching `packages/core/Dockerfile` or `packages/core/templates/**` **does** open a `otto-sandbox` release PR
- [ ] `publish-image.yml` triggers on `otto-sandbox-v*` tags, builds + pushes the image, and is the new primary path
- [ ] After push, the workflow appends the image digest to the corresponding GH Release body in the form `Image: docker.io/phamvuhoang/otto-sandbox@sha256:…`
...
- [ ] After a successful `@phamvuhoang/otto-core` publish, the corresponding GH Release contains the `.tgz` produced by `pnpm pack`, an SBOM file, and a cosign attestation
- [ ] After a successful `@phamvuhoang/otto` publish, the corresponding GH Release contains the same three artifacts
...
- [ ] The compatibility matrix lists known-good combinations of `@phamvuhoang/otto` × `@phamvuhoang/otto-core` × `otto-sandbox` image tag, with at least the current set populated
...
The `otto-sandbox` synthetic component is the most novel and most easily mis-configured piece of `release-please-config.json`. A small contract test runs `release-please --dry-run` (or its programmatic equivalent) against a synthetic commit-history fixture and asserts the proposed manifest update matches expectation. The fixture covers three cases: a `feat:` commit touching only `packages/core/src/**` (must bump `otto-core` only), a commit touching `packages/core/Dockerfile` (must bump `otto-sandbox` only), and a commit touching `apps/cli/**` (must bump `otto` only).
...
- [ ] The test asserts that a `feat:` commit touching only `packages/core/src/**` bumps `otto-core` and not `otto-sandbox`
- [ ] The test asserts that a `feat:` commit touching only `packages/core/Dockerfile` bumps `otto-sandbox` and not `otto-core`
- [ ] The test asserts that a `feat:` commit touching only `apps/cli/**` bumps `otto` and not the others
