# PRD: Multi-Version Node + .NET Runtime Support for otto-sandbox

## Problem Statement

As a Otto user, when I point `otto-afk` or `otto-ghafk` at a target repository, the sandbox container runs whatever Node and .NET versions happen to be baked into the image — currently Node 22 and .NET SDK 10. If my target repo needs Node 20 (because of a transitive dep that doesn't support 22) or .NET 8 (LTS) or any other combination, the iterations fail at `pnpm install` / `dotnet restore` time, or worse, succeed against the wrong runtime and produce code that breaks in CI.

I have many kinds of repos — different .NET LTS lines, different Node majors, different framework versions — and I want Otto to "just work" against each without rebuilding the sandbox image or maintaining a private fork.
...
- Run `otto-afk` against any supported repo. Otto reads the repo's `.nvmrc` / `.tool-versions` / `package.json engines` / `global.json` / `.csproj` and activates the matching runtime inside the container.
...
- If the repo has no manifests, Otto silently falls back to the baked default (latest LTS) and emits a warning.
...
- Framework CLIs (Angular, Next, Nest, React tooling) keep working through the target repo's own `package.json` and `pnpm dlx` / `npx`; Otto doesn't try to globally manage them.

## User Stories

1. As a Otto user with a Node 20 monorepo, I want the sandbox to detect `.nvmrc = 20` and run Node 20 inside the container, so that `pnpm install` succeeds without me overriding anything.
2. As a Otto user with a .NET 8 LTS service, I want the sandbox to detect `global.json` (sdk.version 8.0.x) and use .NET SDK 8.0 inside the container, so that `dotnet build` succeeds against the same toolchain as my CI.
3. As a Otto user with a `.tool-versions` file pinning both `node` and `dotnet`, I want both versions activated simultaneously, so that fullstack iterations work end-to-end.
4. As a Otto user with a `.mise.toml` file, I want it to take precedence over my legacy `.nvmrc`, so that mise-native projects are detected correctly.
5. As a Otto user with a `package.json engines.node = "^22"`, I want the sandbox to pick the highest baked Node satisfying that range, so that I don't pay an install-time cost for a perfectly-acceptable baked version.
6. As a Otto user with a `package.json engines.node = ">=24"`, I want mise to lazy-install Node 24 on the first iteration and cache it across runs, so that subsequent iterations are fast.
7. As a Otto user with a monorepo containing several `.csproj` files all targeting `net8.0`, I want the sandbox to activate .NET 8.0 once for the whole repo, so that all subprojects build.
8. As a Otto user with a monorepo containing `.csproj` files targeting different TFMs, I want the sandbox to pick the highest TFM and emit a warning listing the divergence, so that I'm informed of the tradeoff without being blocked.
9. As a Otto user pointing at a repo with no Node / .NET manifests at all, I want the sandbox to silently fall back to the baked default and continue, so that scripting-only or doc-only repos don't fail to launch.
10. As a Otto user, I want the detected versions surfaced into the NDJSON iteration log, so that I can audit which runtime the agent actually ran against.
11. As a Otto user, I want the published image to be multi-arch (amd64 + arm64), so that I can run Otto on an Apple Silicon laptop without QEMU emulation.
12. As a Otto user on a slow connection, I want the active-LTS versions pre-baked into the image, so that I don't re-download Node and .NET for every fresh container.
13. As a Otto user, I want a docker named volume to cache mise-installed runtimes across `docker run --rm` invocations, so that the second iteration with an exotic Node version is instant.
14. As a Otto maintainer, I want the v2 image published under a `:v2` tag first (not `:latest`), so that I can soak-test it before promoting and breaking existing users.
15. As a Otto maintainer, I want detection logic isolated in a single pure module that can be unit-tested without Docker, so that I can iterate on manifest parsing confidently.
16. As a Otto maintainer, I want the templates (`afk.md`, `ghafk.md`, `prompt.md`, etc.) to remain version-agnostic, so that I don't have to maintain N variants of each playbook.
17. As a Otto user, I want an env-var escape hatch (`OTTO_NODE`, `OTTO_DOTNET`) to override detection, so that I can experiment without modifying the target repo.
18. As a Otto user with a repo whose `engines.node` is `*` or absent, I want detection to gracefully fall through, so that overly-permissive manifests don't crash the iteration.
19. As a Otto user, I want detection to run once per loop invocation, not once per iteration, so that the agent's mid-loop edits to manifests don't change the runtime under it.
20. As a Otto user, I want a clear warning when my requested .NET version isn't baked into the image, so that I understand why the sandbox is falling back to a different SDK.
...
- **`runner` (modify, shallow glue)** — extends the docker invocation with a named-volume mount for the mise cache and two env vars (`OTTO_NODE`, `OTTO_DOTNET`) carrying the detected versions. No host-side `docker volume create` needed; Docker auto-creates named volumes on first use.
...
- **Entrypoint shim (new, ~25-line bash)** — reads `OTTO_NODE` / `OTTO_DOTNET`, activates the Node version via `mise use --global` (lazy-installing if not present), validates the .NET SDK is baked (warns and falls back otherwise — apt-only installs require root and are not safe at runtime), then `exec`s the claude CLI.
...
- `OTTO_NODE` — optional. Exact version, major, or alias (`"lts"`). Empty / unset means "use baked default."
- `OTTO_DOTNET` — optional. Exact version or `major.minor`. Empty / unset means "use baked default."
- `OTTO_MISE_VOLUME` — optional. Override the default `otto-mise-cache` volume name for project-scoped isolation.
...
- A named docker volume (`otto-mise-cache` by default) caches lazy-installed Node versions at `/home/agent/.local/share/mise`. Survives `docker run --rm`.
...
- `@phamvuhoang/otto-core` minor version bump on merge. `IMAGE_REF` default stays `:latest` and naturally inherits the promoted v2 image post-soak.
...
Docker container behavior, mise install behavior, the entrypoint shim's bash, CI workflow YAML. Those get manual smoke tests: `docker run --rm -e OTTO_NODE=20 otto-sandbox:v2 node --version` and similar.
...
- **A `otto.config.json` for per-repo overrides** — not needed initially. Env vars cover the override case; if precedence rules need to be repo-overridable later, that's a separate PRD.
...
2. `pnpm --filter @phamvuhoang/otto-core test` — `detect.ts` unit tests green.
3. `docker build -t otto-sandbox:dev -f packages/core/Dockerfile .` — builds locally.
4. `docker run --rm -e OTTO_NODE=20 otto-sandbox:dev node --version` — prints `v20.x.x`.
5. `docker run --rm -e OTTO_DOTNET=8.0 otto-sandbox:dev dotnet --list-sdks` — shows 8, 9, 10.
6. `otto-afk --print-config` against a temp repo containing `.nvmrc = 20` and `global.json` (sdk 8.0.x) — prints detected versions and image ref.
