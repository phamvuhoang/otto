...
Otto ships three independent artifacts: the `@phamvuhoang/otto-core` npm package, the `@phamvuhoang/otto` CLI npm package, and the `docker.io/phamvuhoang/otto-sandbox` image. Today, releasing any of them is partly manual and partly accidental:
...
- `apps/cli` does **not** publish at all from CI. There is no workflow trigger for it, so the CLI version (currently `0.1.0`) lags `otto-core` (`0.1.1`) silently.
...
2. **publish-npm.yml** (rewritten) — triggered by tags `otto-core-v*` and `otto-v*`. Publishes the matching package to npm. Closes the current CLI publish gap.
3. **publish-image.yml** (rewritten) — triggered by tag `otto-sandbox-v*`. Builds the image, pushes to Docker Hub, captures the digest, writes it back into the GH Release notes.
...
5. As a maintainer, I want `apps/cli` to publish on its own tag, so that the CLI no longer lags `otto-core` silently.
...
10. As a user of `@phamvuhoang/otto`, I want a `CHANGELOG.md` entry per release explaining what changed, so that I can decide whether to upgrade.
11. As a user of `@phamvuhoang/otto`, I want the changelog grouped by `feat:` / `fix:` / `chore:` headings, so that I can scan for breaking changes quickly.
...
20. As a maintainer, I want a tag-naming convention (`otto-core-vX.Y.Z`, `otto-vX.Y.Z`, `otto-sandbox-vX.Y.Z`), so that each workflow can match its own component without false fires.
...
- Three components are declared: `packages/core` (component `otto-core`), `apps/cli` (component `otto`), and a synthetic component `otto-sandbox` rooted at `packages/core` but path-scoped to `packages/core/Dockerfile` and `packages/core/templates/**`. The synthetic component lets the image bump on Dockerfile/template changes without dragging `otto-core` along.
- Tag schema: `otto-core-vX.Y.Z`, `otto-vX.Y.Z`, `otto-sandbox-vX.Y.Z`. The existing `image-v0.1.1` tag is left in place; the manifest is seeded so the first `otto-sandbox` release starts at the next appropriate semver.
...
- **`.github/workflows/publish-npm.yml`** — rewritten. Trigger is `push: tags: ['otto-core-v*', 'otto-v*']`. A single job inspects `${GITHUB_REF_NAME}` to decide whether to publish `packages/core` or `apps/cli`. Existing path-based push trigger is removed (release-please owns the "when to release" decision). `pnpm install --frozen-lockfile` + `pnpm -r build` + `JS-DevTools/npm-publish` continue to power the actual publish step.
- **`.github/workflows/publish-image.yml`** — rewritten. Trigger is `push: tags: ['otto-sandbox-v*']` (the existing `image-v*` trigger is retained for one release as a compatibility shim, then removed in a follow-up). `workflow_dispatch` retained for emergency rebuilds. After push, the workflow reads the image digest from the buildx output and calls `gh release edit` to append `Image: docker.io/phamvuhoang/otto-sandbox@sha256:…` to the release body.
...
8. Tag naming and the synthetic `otto-sandbox` component

### Migration

- Existing artifacts (`@phamvuhoang/otto-core@0.1.1`, `@phamvuhoang/otto@0.1.0`, `image-v0.1.1`) are preserved. The manifest is seeded with those versions so release-please's first PR proposes bumps from those baselines.
...
- Make a Dockerfile change; verify only the `otto-sandbox` component bumps, the image publishes, and the digest is appended to the release notes.
...
- **Automatically opening upgrade PRs in downstream consumer repos** when a new otto version ships. Out of scope.
...
- The synthetic `otto-sandbox` component is the most novel part of the release-please config and the most likely place for misconfiguration. The dry-run contract test exists specifically to catch this.
