...
You can now cap how many times each step retries before giving up, so a stuck run can't keep retrying forever (burning time and money). You turn it on with `--max-retries`.
...
Otto added a `--max-retries` option, parses it into the run config, and clamps invalid retry counts before the loop uses them.
...
You can now run the issue watcher with fewer redundant issue-tracker calls. It reads each issue once per round, making each poll faster and quieter.
...
Otto added a per-poll issue-body cache and clears it at the end of each watch cycle so the next poll still sees fresh issue text.
...
You can now re-run the external review with the two confirmed problems closed: live output stream errors no longer crash the run, and the log file remains readable.
...
Otto added error handling around the stream reader and fixed the NDJSON writer so each log entry terminates with a line break.
...
You can now decide which plan tasks are safe to trust and which need re-opening. Otto found that two checked-off tasks cannot be confirmed from the code.
...
Nothing changed in source code; this read-only verify pass compared the plan against committed evidence and wrote its report to scratch.
