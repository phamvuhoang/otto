...
| `--permission-mode bypassPermissions`                             | no 1:1 — closest is global `--ask-for-approval never` plus `exec --ignore-user-config --sandbox <mode>` (or `--full-auto` / `--dangerously-bypass-approvals-and-sandbox`) | documented                                            |
