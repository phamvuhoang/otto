...
- [ ] **Step 2: Run → fail.** `pnpm --filter @phamvuhoang/otto-core test -- pacing` → module not found.
...
- [ ] **Step 4: Run → pass.** `pnpm --filter @phamvuhoang/otto-core test -- pacing`
...
- [ ] **Step 2: Run → fail.** `pnpm --filter @phamvuhoang/otto-core test -- runner`
...
- [ ] **Step 5: Run → fail (loop.ts/loop.test still expect string).** Expected — fixed in Task 3. Run `pnpm --filter @phamvuhoang/otto-core test -- runner` to confirm the runner tests themselves pass; typecheck will flag loop.ts until Task 3.
...
- [ ] **Step 2: Run → fail.** `pnpm --filter @phamvuhoang/otto-core test -- loop`
...
- [ ] **Step 4: Run → pass.** `pnpm --filter @phamvuhoang/otto-core test -- loop` and `-- runner`.
...
- [ ] **Step 4: Verify.** `pnpm -r typecheck && pnpm -r test`, then `node apps/cli/bin/otto-afk.js --print-config --budget 2.5 --cooldown 1500 x 1` shows `budget $2.50`, `cooldown 1500ms`.
...
- [ ] **Step 3: Run the FULL existing loop suite → must stay green** (retry, failure, abort, render-failure tests): `pnpm --filter @phamvuhoang/otto-core test -- loop`. If any retry/failure/abort assertion breaks, the extraction changed behavior — fix until identical.
...
export async function runPanel(opts: RunPanelOptions): Promise<StageResult> {
  ...
  try {
    ...
    const synth = await executeStage({
      stage: SYNTH_STAGE,
      vars: { FINDINGS_DIR: `./${posix.join(".otto-tmp", panelRel)}/` },
      workspaceDir,
      packageDir,
      iteration,
      maxRetries,
...
- [ ] **Step 4: Run → pass.** `pnpm --filter @phamvuhoang/otto-core test -- panel`
...
- [ ] **Step 2: CLI flag.** In `cli-help.ts` `parseFlags`, add `reviewPanel: boolean` (set by `--review-panel`). In `printHelp`, document `--review-panel` and the `OTTO_REVIEW_LENSES` env var. In `printConfig`, add a `review` line:
...
- [ ] **Step 4: Verify.** `pnpm -r typecheck && pnpm -r test && pnpm test`. Then:
      `node apps/cli/bin/otto-afk.js --print-config --review-panel x 1` → `review  panel: correctness, security, tests`.
      `OTTO_REVIEW_LENSES=correctness node apps/cli/bin/otto-afk.js --print-config x 1` → `review  panel: correctness`.

- [ ] **Step 5: Commit** — `git commit -am "feat: wire --review-panel / OTTO_REVIEW_LENSES into the loop"`
...
- [ ] **Step 2: Budget/cooldown smoke (no model call needed).** `node apps/cli/bin/otto-afk.js --print-config --budget 5 --cooldown 2000 --review-panel x 1` shows all three resolved.

- [ ] **Step 3: Live panel smoke (real claude; requires auth — surface to the human).** In a throwaway repo with one trivial commit:

```bash
OTTO_WORKSPACE=/tmp/otto-smoke node apps/cli/bin/otto-afk.js --review-panel --cooldown 1000 "Read plan.md and create the file it describes" 1
```

Expect: implementer stage, then **three lens stages** (correctness/security/tests, read-only — no commits), ~1s cooldowns between them, then one synth stage that emits `<review>OK</review>` or a single `fix(review):` commit. A running cost line prints per stage. Inspect `.otto-tmp/logs/*lens-*.ndjson` + confirm the `panel-*` dir was created and cleaned.
