...
**Goal:** Give Otto a persistent, git-tracked `.otto/LEARNINGS.md` memory file that is injected into every stage's prompt and appended to inline by the agent, so durable repo knowledge carries across iterations.
...
- **Modify** `CLAUDE.md` — note `.otto/LEARNINGS.md` in the scratch-dir section.
...
git commit -m "feat(learnings): inject .otto/LEARNINGS.md into stage prompts"
```
...
Run: `pnpm --filter @phamvuhoang/otto-core test -- render learnings`
...
```
**Learning loop.** Otto keeps a git-tracked `.otto/LEARNINGS.md` in the target repo. Its contents are injected into every implementer/reviewer prompt, and the agent appends durable, reusable learnings (repo conventions, gotchas, decisions, dead ends) to it as it works — so knowledge accumulates across iterations instead of being relearned each run. The file is committed alongside the work; delete it to reset Otto's memory.
```
...
git commit -m "docs(learnings): document the .otto/LEARNINGS.md memory file"
```
...
```bash
mkdir -p /tmp/otto-smoke && cd /tmp/otto-smoke && git init -q && git commit -q --allow-empty -m "init"
# from a shell where `otto-afk` is installed (see CLAUDE.md smoke-test section)
OTTO_WORKSPACE=/tmp/otto-smoke otto-afk "Create a file hello.txt containing 'hi', then there are no more tasks. Note in learnings that this repo has no build step." 2
```

- [ ] **Step 2: Verify the file was created, committed, and fed back**

```bash
cat /tmp/otto-smoke/.otto/LEARNINGS.md          # exists, has a learning
git -C /tmp/otto-smoke log --oneline             # LEARNINGS.md rode in a work commit
grep -l "no build step" /tmp/otto-smoke/.otto-tmp/.run-*.md 2>/dev/null || \
  grep -r "no build step" /tmp/otto-smoke/.otto-tmp/logs/   # appears in iter-2 prompt
```

Expected: `.otto/LEARNINGS.md` exists with the recorded learning; it appears in a commit; its content appears in the second iteration's rendered prompt or NDJSON log.
