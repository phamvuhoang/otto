...
**Goal:** Replace Otto's Docker sandbox with Claude Code's native OS (Seatbelt) sandbox, running `claude` directly on the host, and delete all Docker plumbing.

**Architecture:** `runStage` spawns `claude --print …` with `cwd = workspaceDir` instead of `docker run …`. A new `OTTO_RUNNER` selects `sandbox` (default — writes a transient `--settings` JSON enabling the Seatbelt sandbox with writes confined to the workspace) or `host` (no sandbox, bare while-loop). `ensureImage`, socket detection, credential mounts, and the image/Dockerfile/CI are removed.
...
| `packages/core/src/loop.ts`                  | Iteration driver. No `ensureImage`, no `ottoDir`.                                                                                                                                                                |
| `packages/core/src/run-bin.ts`               | Resolve dirs (no `ottoDir`), drive `runLoop`.                                                                                                                                                                    |
...
| Deleted                                      | `templates/Dockerfile`, `templates/CHANGELOG.md`, `.github/workflows/publish-image.yml`, release-please `otto-sandbox` component, SECURITY.md docker.sock section.                                               |
...
## Task 6: Drop `ensureImage`/`ottoDir` from the loop (TDD)
...
(b) In `loopOptions` (lines 86–96), delete the `ottoDir: dirs.ottoDir,` line. In `LoopDirs`/`makeDirs`, delete `ottoDir` (lines 64, 70, 75) — it is no longer used.
...
Expected: FAIL — `runLoop` still calls `ensureImage` / `LoopOptions` still requires `ottoDir`.
...
(b) In `LoopOptions`, delete the `ottoDir` field + its doc comment (lines 38–39).

(c) In the `runLoop` destructure, delete `ottoDir,` (line 61).

(d) Delete the `await ensureImage(ottoDir, { signal: stageAbort.signal });` line (line 111).
...
git commit -m "refactor(loop): drop image setup and ottoDir; sandbox needs no image"
```

---

## Task 7: Drop `ottoDir`/`OTTO_DOCKER_CONTEXT` from `run-bin.ts`

**Files:**

- Modify: `packages/core/src/run-bin.ts`

- [ ] **Step 1: Remove the ottoDir resolution + pass-through**

(a) Delete line 54: `const ottoDir = resolve(process.env.OTTO_DOCKER_CONTEXT ?? packageDir);`

(b) Update the `printConfig` call (lines 62–70) to drop the `ottoDir` argument:
...
(c) In the `runLoop({…})` call (lines 94–106), delete the `ottoDir,` line.
...
Replace the whole `printConfig` (and its `PrintConfigOptions` stays as-is) with a Docker-free version. New signature drops `ottoDir`:
...
Replace the `// All stages run inside the ephemeral otto-sandbox container …` comment (lines 7–15) with:
...
- Modify: release-please config (remove the `otto-sandbox` component)
...
```bash
grep -rln "otto-sandbox\|Dockerfile\|publish-image\|templates/CHANGELOG" \
...
```bash
grep -rn "otto-sandbox\|publish-image\|docker build\|OTTO_DOCKER" .github release-please-config.json .release-please-manifest.json packages apps || echo "clean"
```
...
```bash
grep -rniE "docker|otto-sandbox|OTTO_IMAGE|OTTO_DOCKER|container|dockerfile|docker.sock|bind-mount" \
  CLAUDE.md README.md RELEASING.md SECURITY.md docs/ARCHITECTURE.md
```

- [ ] **Step 2: Rewrite each hit to the host/sandbox model**

For each file, replace the Docker runtime description with: "Otto runs `claude` directly on the host; the default `OTTO_RUNNER=sandbox` uses Claude Code's native OS sandbox (writes confined to the workspace), `OTTO_RUNNER=host` runs unsandboxed." Specifically:

- **CLAUDE.md** — "What this repo is", the `runStage`/`ensureImage`/runner bullets, the env-knob list (drop `OTTO_IMAGE`/`OTTO_DOCKER_*`, add `OTTO_RUNNER`/`OTTO_SANDBOX_NET`), "Credential mounts" → "Credentials" (host reads `~/.claude`, `~/.config/gh` natively), the "Building / publishing the sandbox image" section (delete), "Sandbox image" (delete).
- **README.md** — install/first-run no longer needs Docker; remove image pull/build troubleshooting; document `OTTO_RUNNER`/`OTTO_SANDBOX_NET`; keep the macOS Seatbelt note.
- **docs/ARCHITECTURE.md** — runtime model: spawn `claude` not `docker run`; remove the image-lifecycle + socket sections.
- **RELEASING.md** — remove the image release component; releases are npm-only now (`@phamvuhoang/otto-core` + `@phamvuhoang/otto`).
...
```bash
grep -rniE "OTTO_IMAGE|OTTO_DOCKER|otto-sandbox|docker run|docker pull|docker.sock" \
...
```bash
cd /tmp && rm -rf otto-smoke && mkdir otto-smoke && cd otto-smoke && git init -q && echo "# plan: create HELLO.md containing 'hi'" > plan.md && git add -A && git commit -qm init
OTTO_WORKSPACE=/tmp/otto-smoke otto-afk "Read plan.md and do it" 1
```

Expected: a stage runs with **no `docker` process** (confirm `docker ps` shows nothing new / `pgrep -fl docker` unchanged), `HELLO.md` is created, and a commit lands. Inspect `/tmp/otto-smoke/.otto-tmp/logs/*.ndjson` for the stream and confirm a `.sandbox-*.json` was written + cleaned.

- [ ] **Step 4: Smoke test the `host` runner**

```bash
OTTO_RUNNER=host OTTO_WORKSPACE=/tmp/otto-smoke otto-afk --print-config "x" 1
```

Expected: `--print-config` prints `OTTO_RUNNER  host` and `sandbox network  n/a (host runner)`.
