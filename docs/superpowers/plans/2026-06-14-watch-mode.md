...
**Goal:** Add a polling daemon mode to `otto-ghafk` (`--watch`): idle → poll for labelled open issues → run the loop → idle, with daemon-cumulative budget and clean Ctrl+C.
...
- [ ] **Step 2: Run → fail.** `pnpm --filter @phamvuhoang/otto-core test -- retry`
...
- [ ] **Step 4: Run → pass.** `pnpm --filter @phamvuhoang/otto-core test -- watch`
...
- [ ] **Step 1: `cli-help.ts`.** Add `watch: boolean; watchIntervalSec?: number;` to `CliFlags`. Parse `--watch` (boolean) and `--watch-interval <sec>` (expecting-value, validate `/^\d+$/` and `> 0`, else throw `--watch-interval must be a positive integer (seconds), got: …`). Document both in `printHelp` plus the `OTTO_WATCH_LABEL` env var. In `PrintConfigOptions` add `watch?: boolean; watchIntervalSec?: number;` and print:

```
  watch                 ${watch ? `on (every ${watchIntervalSec ?? 300}s, label "${process.env.OTTO_WATCH_LABEL?.trim() || "otto"}")` : "off"}
```
...
node apps/cli/bin/otto-ghafk.js --print-config --watch --watch-interval 120 1   # → watch on (every 120s, label "otto")
node apps/cli/bin/otto-afk.js --watch x 1   # → errors: --watch is only supported by otto-ghafk
```
...
- [ ] **Step 2: print-config matrix** (no model calls):
  - `otto-ghafk --print-config --watch 1` → `watch  on (every 300s, label "otto")`.
  - `OTTO_WATCH_LABEL=bot otto-ghafk --print-config --watch --watch-interval 30 1` → label "bot", every 30s.
  - `otto-afk --watch x 1` → ghafk-only error.

- [ ] **Step 3: Live watch smoke (needs a gh-authed repo; surface to the human).** In a throwaway repo with the `gh` remote: `OTTO_WATCH_LABEL=otto otto-ghafk --watch --watch-interval 15 1`. With **no** labelled issues, it should idle ("0 open issues… ") and poll every 15s; add a `otto`-labelled issue → next poll runs the loop; `Ctrl+C` → clean exit, wake-lock released. (Cheap to validate idle/poll without any model call by just leaving it label-less for two polls then Ctrl+C.)
