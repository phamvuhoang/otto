...
const expectOnly = (files, component) => {
  ...
  for (const other of ["otto-core", "otto"]) {
...
test("config declares the two expected components", () => {
  ...
  assert.deepEqual(new Set(components), new Set(["otto-core", "otto"]));
  assert.deepEqual(
...
test("feat touching only packages/core/src/** bumps otto-core", () => {
  expectOnly(["packages/core/src/loop.ts"], "otto-core");
  expectOnly(
...
test("change touching packages/core/templates/** (playbooks) bumps otto-core", () => {
  // Templates ship inside the @phamvuhoang/otto-core npm tarball, so a playbook
  // edit must release a new otto-core (no separate image component anymore).
  expectOnly(["packages/core/templates/prompt.md"], "otto-core");
});

test("feat touching only apps/cli/** bumps otto, not the others", () => {
  expectOnly(["apps/cli/bin/otto-afk.js"], "otto");
});
